...
public class MostrarAdoptados extends JDialog {
	...
	static JPanel panelFechaAdopcion;
	...
	public static void main(String[] args) {
		try {
			MostrarAdoptados dialog = new MostrarAdoptados();
	 ...
	 */
	public MostrarAdoptados() throws ListaVaciaException {
		...
		contentPanel.add(button_1);
		{
			...
			panelFechaAdopcion.setBounds(114, 340, 144, 43);
	...
	/**
	 * Método para obtener el nombre de la clase del animal
	 * @param tmp
	 * @return
	 */
	...
	/**
	 * Método para rellenar los datos del animal
	 * @throws ListaVaciaException en caso de que la lista esté vacía
	 */
	static void rellenarCampos() throws ListaVaciaException{


		tmp = Principal.refugio.getListaAdoptados();
		if(tmp.size() != 0){

			pesoTextField.setText(Float.toString(tmp.get(i).getPeso()));
			edadTextField_2.setText(Integer.toString(tmp.get(i).getEdad()));
			patastextField_4.setText(Integer.toString(tmp.get(i).getnPatas()));
			reproducciontextField_8.setText(tmp.get(i).getReproduccion().toString());
			alimentaciontextField_9.setText(tmp.get(i).getAlimentacion().toString());
			costeAdopciontextField_7.setText(Float.toString(tmp.get(i).getCosteAdopcion()));
			costeMensuatextField_6.setText(Float.toString(tmp.get(i).getCosteMantenimientoMensual()));
			familiaTextField_10.setText(tmp.get(i).getTipoAnimal().toString());
			tipoAnimaltextField_11.setText(getNombreClase(tmp.get(i)));
			sexoTextField_3.setText(tmp.get(i).getSexo().toString());
			nombretextField.setText(tmp.get(i).getNombre());

			if(tmp.get(i).getAdoptado())
				adoptadoCheckbox.setSelected(true);
			else
				adoptadoCheckbox.setSelected(false);
			Calendar cal = tmp.get(i).getFechaAlta();
			...
			if(tmp.get(i).estaAdoptado()){
				panelFechaAdopcion.setVisible(true);
				String formateado = sdf.format(tmp.get(i).getFechaAdopcion().getTime());
