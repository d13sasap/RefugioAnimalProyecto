...
public class MostrarAves extends JDialog {
	...
	static JCheckBox volarCheckBox;
	static JCheckBox plumasCheckBox;
	...
	static int i;
	private static ArrayList<Ave> listaAves = null;
	private static JTextField fechaAdopciontextField;
	static JPanel fechaAdopcionpanel;

	private static ArrayList<Ave> crearListaAves(int opcion) {
		if(opcion == 0)
			return refugio.getlistaAves();
		if(opcion == 1)
			return refugio.getlistaPalomas();

		return refugio.getListaLoros();
	}
	...
	public static void main(String[] args) {
		try {
			MostrarAves dialog = new MostrarAves(opcion);
	...
	public MostrarAves(int opcionElegida) {
		setTitle("Mostrar Aves");
		setResizable(false);
		setModal(true);
		i = 0;
		this.opcion = opcionElegida;
		listaAves = crearListaAves(opcion);
		...
		volarCheckBox = new JCheckBox("Vuela");
		volarCheckBox.setEnabled(false);
		volarCheckBox.setBounds(305, 355, 97, 23);
		contentPanel.add(volarCheckBox);

		plumasCheckBox = new JCheckBox("Plumas");
		plumasCheckBox.setEnabled(false);
		plumasCheckBox.setBounds(419, 355, 97, 23);
		contentPanel.add(plumasCheckBox);
	...
	private static String getNombreClase(Ave tmp) {
	...
	static void rellenarCampos(){
		...
		if(tmp != null){
			...
			volarCheckBox.setSelected(tmp.isVuela());
			plumasCheckBox.setSelected(tmp.isPlumas());
