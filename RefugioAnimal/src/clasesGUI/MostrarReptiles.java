...
import clasesPrincipales.Reptil;
...
public class MostrarReptiles extends JDialog {
	...
	static JCheckBox caparaznCheckBox;
	static JCheckBox venenosoCheckBox;
	static Reptil tmp = null;
	JLabel lblNombre;
	static int opcion = 0;
	static JCheckBox adoptadoCheckbox = null;
	static int i;
	private static ArrayList<Reptil> listaReptiles = null;
	private static JTextField fechaAdopcionextField;
	static JPanel fechaAdopcionpanel;

	private static ArrayList<Reptil> crearListaReptiles(int opcion) {
		if(opcion == 0)
			return refugio.getlistaReptiles();
		if(opcion == 1)
			return refugio.getlistaTortugas();

		return refugio.getListaSerpientes();
	}
	...
	public static void main(String[] args) {
		try {
			MostrarReptiles dialog = new MostrarReptiles(opcion);
	...
	public MostrarReptiles(int opcionElegida) {
		setTitle("Reptiles");
		setResizable(false);
		setModal(true);
		i = 0;
		this.opcion = opcionElegida;
		listaReptiles = crearListaReptiles(opcion);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(i>0){//me creaba problemas la fecha al avanzar /volver atrás en la lista
					if(i>0)
						i--;
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(i<refugio.getTamanioRefugio()-1){
					if(i<listaReptiles.size()-1)
						i++;
					if(i!=listaReptiles.size()){
		...
		caparaznCheckBox = new JCheckBox("Caparaz\u00F3n");
		caparaznCheckBox.setEnabled(false);
		caparaznCheckBox.setBounds(281, 355, 97, 23);
		contentPanel.add(caparaznCheckBox);

		venenosoCheckBox = new JCheckBox("Venenoso");
		venenosoCheckBox.setEnabled(false);
		venenosoCheckBox.setBounds(395, 355, 97, 23);
		contentPanel.add(venenosoCheckBox);
		{
			fechaAdopcionpanel = new JPanel();
			fechaAdopcionpanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Fecha Adopcion", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
			fechaAdopcionpanel.setBounds(114, 340, 144, 43);
			contentPanel.add(fechaAdopcionpanel);
			fechaAdopcionpanel.setLayout(null);
			{
				fechaAdopcionextField = new JTextField();
				fechaAdopcionextField.setEditable(false);
				fechaAdopcionextField.setBounds(6, 16, 132, 20);
				fechaAdopcionpanel.add(fechaAdopcionextField);
				fechaAdopcionextField.setColumns(10);
			}
		}
		{
	...
	private void establecerTitulo(int opcion2) {
		if(opcion == 0)
			setTitle("Mostrar Reptiles");
		else if(opcion == 1)
			setTitle("Mostrar Tortugas");
		else
			setTitle("Mostrar Serpientes");
	...
	private static String getNombreClase(Reptil tmp) {
	...
	static void rellenarCampos(){
		...
		if(tmp != null){
			...
			caparaznCheckBox.setSelected(tmp.isCaparazon());
			venenosoCheckBox.setSelected(tmp.isVenenoso());
			...
			if(tmp.estaAdoptado()){
				...
				fechaAdopcionextField.setText(formateado);
			}
			else
